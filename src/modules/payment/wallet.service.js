...
async function getOrCreateWallet(userId, transaction = null, useLock = false) {
  try {
    const findOptions = {
      where: { user_id: userId },
      ...(transaction ? { transaction } : {}),
      ...(transaction && useLock ? { lock: transaction.LOCK.UPDATE } : {}),
    };
...
export async function creditWalletService(userId, amount, reason, externalTransaction = null) {
  ...
  try {
    ...
    const wallet = await getOrCreateWallet(userId, t, true);
