  ...
  const deliveries = result.rows.map(booking => {
    ...
    return {
      ...
      bookingId:
        booking.booking_ref ||
        booking.bookingRef ||
        `TEMP-${booking.id.substring(0, 8).toUpperCase()}`,
      trackingId:
        booking.tracking_ref ||
        booking.trackingRef ||
        `TEMP-${booking.id.substring(0, 12).toUpperCase()}`,
      ...
      paymentMode: booking.payment_mode || "PAY_NOW", // ✅ Default to PAY_NOW (safer default)
      // OTP information
...
export async function fetchTravellerParcelRequests(travellerUserId, query = {}) {
  try {
    ...
    const requests = result.rows.map(request => {
      ...
      return {
        id: request.id,
        parcelId: parcel?.parcel_ref || parcel?.id,
...
export async function fetchTravellerStats(travellerUserId) {
  ...
  const bookings = await Booking.findAll({
    ...
    attributes: ["id", "status", "payment_mode", "createdAt"],
    include: [{
      model: Parcel,
      as: "parcel",
  ...
  bookings.forEach(booking => {
    ...
    if (["CREATED", "MATCHING", "CONFIRMED", "IN_TRANSIT"].includes(status)) {
      ...
      // ✅ PAY_AFTER_DELIVERY awaiting payment — don't count earnings yet
      stats.active += 1;
  ...
  try {
    const pendingPayments = await PendingPayment.findAll({
      ...
      include: [{
        model: Booking,
        as: "booking",
        attributes: ["booking_ref", "delivery_ref", "status"],
        required: true,
      }],
      order: [["createdAt", "DESC"]],
    });
    ...
    return pendingPayments.map(payment => ({
      ...
      delivery_ref: payment.booking?.delivery_ref,
